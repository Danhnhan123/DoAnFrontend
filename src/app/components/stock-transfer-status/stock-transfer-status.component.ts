...
import {
  StockTransferStatusAdvancedRow,
  StockTransferStatusDetailDto,
  CreateStockTransferStatusDto,
  UpdateStockTransferStatusDto,
} from '../../models';
import { StockTransferStatusService } from '../../services/stock-transfer-status.service';
...
@Component({
  selector: 'app-stock-transfer-status',
  standalone: true,
  imports: [ReadonlyIfDirective, HasPermissionDirective, CommonModule, FormsModule, FilterSelectComponent],
  templateUrl: './stock-transfer-status.component.html',
  styleUrl: './stock-transfer-status.component.css',
})
export class StockTransferStatusComponent {
  perm = inject(PermissionService);
  viewOnly = computed(() => this.isEdit() && !this.perm.canUpdate('STOCK_TRANSFER_STATUS'));
  private svc = inject(StockTransferStatusService);
  ...
  editItem = signal<StockTransferStatusAdvancedRow | null>(null);
  ...
  detailQuery = injectQuery(() => ({
    queryKey: ['stock-transfer-status-detail', this.editItem()?.id],
    enabled: !!this.editItem()?.id && this.showModal(),
    queryFn: () => lastValueFrom(this.svc.getById(this.editItem()!.id)),
  }));

  rows = computed<StockTransferStatusAdvancedRow[]>(() => {
  ...
  get _detailSynced(): boolean {
    ...
    if (d && d !== this._prevDetailData) {
      ...
      const detail: StockTransferStatusDetailDto = (d as any)?.resources ?? (d as any)?.data;
  ...
  createMutation = injectMutation(() => ({
    mutationFn: (payload: CreateStockTransferStatusDto) => lastValueFrom(this.svc.create(payload)),
    onSuccess: (res: any) => {
      if (res.isSucceeded) {
        this.closeModal();
        this.queryClient.invalidateQueries({ queryKey: ['stock-transfer-status'] });
  ...
  updateMutation = injectMutation(() => ({
    mutationFn: (payload: UpdateStockTransferStatusDto) => lastValueFrom(this.svc.update(payload)),
    onSuccess: (res: any) => {
      if (res.isSucceeded) {
        this.closeModal();
        this.queryClient.invalidateQueries({ queryKey: ['stock-transfer-status'] });
  ...
  openEdit(row: StockTransferStatusAdvancedRow): void {
    this._prevDetailData = null;
  ...
  save(): void {
    ...
    if (!f.name) { this.showAlert('Vui lòng điền đầy đủ tên trạng thái điều chuyển (*)', false); return; }
    const actionText = this.isEdit() ? 'cập nhật' : 'thêm mới';
    Swal.fire({
      title: `Xác nhận ${actionText}`,
      text: `Bạn có chắc chắn muốn ${actionText} trạng thái điều chuyển này?`,
      icon: 'question',
      ...
      if (this.isEdit()) {
        this.updateMutation.mutate({
          ...
          name: f.name,
          color: f.color,
        } as UpdateStockTransferStatusDto);
      } else {
        this.createMutation.mutate({
          code: f.code,
          name: f.name,
          color: f.color,
        } as CreateStockTransferStatusDto);
      }
    });
  }
  ...
  delete(id: number, name: string): void {
    Swal.fire({
      ...
      text: `Bạn chuẩn bị xóa trạng thái điều chuyển "${name}". Thao tác này không thể hoàn tác!`,
      icon: 'warning',
      showCancelButton: true,
