...
import {
  SalesOrderStatusAdvancedRow,
  SalesOrderStatusDetailDto,
  CreateSalesOrderStatusDto,
  UpdateSalesOrderStatusDto,
} from '../../models';
import { SalesOrderStatusService } from '../../services/sales-order-status.service';
...
@Component({
  ...
  styleUrl: './sales-order-status.component.css',
})
export class SalesOrderStatusComponent {
  perm = inject(PermissionService);
  viewOnly = computed(() => this.isEdit() && !this.perm.canUpdate('SALES_ORDER_STATUS'));
  private svc = inject(SalesOrderStatusService);
  ...
  createMutation = injectMutation(() => ({
    mutationFn: (payload: CreateSalesOrderStatusDto) => lastValueFrom(this.svc.create(payload)),
    onSuccess: (res: any) => {
      if (res.isSucceeded) {
        this.closeModal();
        this.queryClient.invalidateQueries({ queryKey: ['sales-order-status'] });
  ...
  updateMutation = injectMutation(() => ({
    mutationFn: (payload: UpdateSalesOrderStatusDto) => lastValueFrom(this.svc.update(payload)),
    onSuccess: (res: any) => {
      if (res.isSucceeded) {
        this.closeModal();
        this.queryClient.invalidateQueries({ queryKey: ['sales-order-status'] });
  ...
  save(): void {
    ...
    Swal.fire({
      ...
      text: `Bạn có chắc chắn muốn ${actionText} trạng thái đơn bán này?`,
      icon: 'question',
  ...
  delete(id: number, name: string): void {
    Swal.fire({
      ...
      text: `Bạn chuẩn bị xóa trạng thái đơn bán "${name}". Thao tác này không thể hoàn tác!`,
      icon: 'warning',
      showCancelButton: true,
