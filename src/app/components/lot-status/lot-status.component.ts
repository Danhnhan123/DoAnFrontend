...
import {
  LotStatusAdvancedRow,
  LotStatusDetailDto,
  CreateLotStatusDto,
  UpdateLotStatusDto,
} from '../../models';
import { LotStatusService } from '../../services/lot-status.service';
...
export class LotStatusComponent {
  perm = inject(PermissionService);
  viewOnly = computed(() => this.isEdit() && !this.perm.canUpdate('LOT_STATUS'));
  private svc = inject(LotStatusService);
  ...
  editItem = signal<LotStatusAdvancedRow | null>(null);
  isEdit = computed(() => !!this.editItem());
  form = signal<any>({ code: '', name: '', color: '#16a34a', isSellable: true });
  ...
  get _detailSynced(): boolean {
    ...
    if (d && d !== this._prevDetailData) {
      ...
      const detail: LotStatusDetailDto = (d as any)?.resources ?? (d as any)?.data;
      if (detail) this.form.set({ code: detail.code || '', name: detail.name, color: detail.color, isSellable: detail.isSellable ?? true });
    }
    return true;
  }

  createMutation = injectMutation(() => ({
    mutationFn: (payload: CreateLotStatusDto) => lastValueFrom(this.svc.create(payload)),
    onSuccess: (res: any) => {
      if (res.isSucceeded) {
        this.closeModal();
        this.queryClient.invalidateQueries({ queryKey: ['lot-status'] });
  ...
  updateMutation = injectMutation(() => ({
    mutationFn: (payload: UpdateLotStatusDto) => lastValueFrom(this.svc.update(payload)),
    onSuccess: (res: any) => {
      if (res.isSucceeded) {
        this.closeModal();
        this.queryClient.invalidateQueries({ queryKey: ['lot-status'] });
  ...
  openCreate(): void {
    ...
    this.form.set({ code: '', name: '', color: '#16a34a', isSellable: true });
    this.showModal.set(true);
  }
  openEdit(row: LotStatusAdvancedRow): void {
    this._prevDetailData = null;
    this.editItem.set(row);
    this.form.set({ code: row.code || '', name: row.name, color: row.color, isSellable: row.isSellable ?? true });
    this.showModal.set(true);
  }
  ...
  save(): void {
    const f = this.form();
    if (!f.name) { this.showAlert('Vui lòng điền đầy đủ tên trạng thái lô (*)', false); return; }
    const actionText = this.isEdit() ? 'cập nhật' : 'thêm mới';
    Swal.fire({
      title: `Xác nhận ${actionText}`,
      text: `Bạn có chắc chắn muốn ${actionText} trạng thái lô này?`,
      icon: 'question',
  ...
  delete(id: number, name: string): void {
    Swal.fire({
      ...
      text: `Bạn chuẩn bị xóa trạng thái lô "${name}". Thao tác này không thể hoàn tác!`,
      icon: 'warning',
      showCancelButton: true,
