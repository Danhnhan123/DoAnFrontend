...
import {
  InboundOrderStatusAdvancedRow,
  InboundOrderStatusDetailDto,
  CreateInboundOrderStatusDto,
  UpdateInboundOrderStatusDto,
} from '../../models';
import { InboundOrderStatusService } from '../../services/inbound-order-status.service';
...
@Component({
  selector: 'app-inbound-order-status',
  standalone: true,
  imports: [ReadonlyIfDirective, HasPermissionDirective, CommonModule, FormsModule, FilterSelectComponent],
  templateUrl: './inbound-order-status.component.html',
  styleUrl: './inbound-order-status.component.css',
})
export class InboundOrderStatusComponent {
  perm = inject(PermissionService);
  viewOnly = computed(() => this.isEdit() && !this.perm.canUpdate('INBOUND_ORDER_STATUS'));
  private svc = inject(InboundOrderStatusService);
  ...
  editItem = signal<InboundOrderStatusAdvancedRow | null>(null);
  ...
  detailQuery = injectQuery(() => ({
    queryKey: ['inbound-order-status-detail', this.editItem()?.id],
    enabled: !!this.editItem()?.id && this.showModal(),
    queryFn: () => lastValueFrom(this.svc.getById(this.editItem()!.id)),
  }));

  rows = computed<InboundOrderStatusAdvancedRow[]>(() => {
  ...
  get _detailSynced(): boolean {
    ...
    if (d && d !== this._prevDetailData) {
      ...
      const detail: InboundOrderStatusDetailDto = (d as any)?.resources ?? (d as any)?.data;
  ...
  createMutation = injectMutation(() => ({
    mutationFn: (payload: CreateInboundOrderStatusDto) => lastValueFrom(this.svc.create(payload)),
    onSuccess: (res: any) => {
      if (res.isSucceeded) {
        this.closeModal();
        this.queryClient.invalidateQueries({ queryKey: ['inbound-order-status'] });
  ...
  updateMutation = injectMutation(() => ({
    mutationFn: (payload: UpdateInboundOrderStatusDto) => lastValueFrom(this.svc.update(payload)),
    onSuccess: (res: any) => {
      if (res.isSucceeded) {
        this.closeModal();
        this.queryClient.invalidateQueries({ queryKey: ['inbound-order-status'] });
  ...
  openEdit(row: InboundOrderStatusAdvancedRow): void {
    this._prevDetailData = null;
  ...
  save(): void {
    const f = this.form();
    if (!f.name) { this.showAlert('Vui lòng điền đầy đủ tên trạng thái đơn nhập (*)', false); return; }
    const actionText = this.isEdit() ? 'cập nhật' : 'thêm mới';
    Swal.fire({
      title: `Xác nhận ${actionText}`,
      text: `Bạn có chắc chắn muốn ${actionText} trạng thái đơn nhập này?`,
      icon: 'question',
      ...
      if (this.isEdit()) {
        this.updateMutation.mutate({
          ...
          name: f.name,
          color: f.color,
        } as UpdateInboundOrderStatusDto);
      } else {
        this.createMutation.mutate({
          name: f.name,
          color: f.color,
        } as CreateInboundOrderStatusDto);
      }
    });
  }
  ...
  delete(id: number, name: string): void {
    Swal.fire({
      ...
      text: `Bạn chuẩn bị xóa trạng thái đơn nhập "${name}". Thao tác này không thể hoàn tác!`,
      icon: 'warning',
      showCancelButton: true,
