...
import {
  MillingOrderStatusAdvancedRow,
  MillingOrderStatusDetailDto,
  CreateMillingOrderStatusDto,
  UpdateMillingOrderStatusDto,
} from '../../models';
import { MillingOrderStatusService } from '../../services/milling-order-status.service';
...
@Component({
  selector: 'app-milling-order-status',
  standalone: true,
  imports: [ReadonlyIfDirective, HasPermissionDirective, CommonModule, FormsModule, FilterSelectComponent],
  templateUrl: './milling-order-status.component.html',
  styleUrl: './milling-order-status.component.css',
})
export class MillingOrderStatusComponent {
  perm = inject(PermissionService);
  viewOnly = computed(() => this.isEdit() && !this.perm.canUpdate('MILLING_ORDER_STATUS'));
  private svc = inject(MillingOrderStatusService);
  ...
  editItem = signal<MillingOrderStatusAdvancedRow | null>(null);
  ...
  detailQuery = injectQuery(() => ({
    queryKey: ['milling-order-status-detail', this.editItem()?.id],
    enabled: !!this.editItem()?.id && this.showModal(),
    queryFn: () => lastValueFrom(this.svc.getById(this.editItem()!.id)),
  }));

  rows = computed<MillingOrderStatusAdvancedRow[]>(() => {
  ...
  get _detailSynced(): boolean {
    ...
    if (d && d !== this._prevDetailData) {
      ...
      const detail: MillingOrderStatusDetailDto = (d as any)?.resources ?? (d as any)?.data;
  ...
  createMutation = injectMutation(() => ({
    mutationFn: (payload: CreateMillingOrderStatusDto) => lastValueFrom(this.svc.create(payload)),
    onSuccess: (res: any) => {
      if (res.isSucceeded) {
        this.closeModal();
        this.queryClient.invalidateQueries({ queryKey: ['milling-order-status'] });
  ...
  updateMutation = injectMutation(() => ({
    mutationFn: (payload: UpdateMillingOrderStatusDto) => lastValueFrom(this.svc.update(payload)),
    onSuccess: (res: any) => {
      if (res.isSucceeded) {
        this.closeModal();
        this.queryClient.invalidateQueries({ queryKey: ['milling-order-status'] });
  ...
  openEdit(row: MillingOrderStatusAdvancedRow): void {
    this._prevDetailData = null;
  ...
  save(): void {
    const f = this.form();
    if (!f.name) { this.showAlert('Vui lòng điền đầy đủ tên trạng thái lệnh xay (*)', false); return; }
    const actionText = this.isEdit() ? 'cập nhật' : 'thêm mới';
    Swal.fire({
      title: `Xác nhận ${actionText}`,
      text: `Bạn có chắc chắn muốn ${actionText} trạng thái lệnh xay này?`,
      icon: 'question',
      ...
      if (this.isEdit()) {
        this.updateMutation.mutate({
          ...
          name: f.name,
          color: f.color,
        } as UpdateMillingOrderStatusDto);
      } else {
        this.createMutation.mutate({
          code: f.code,
          name: f.name,
          color: f.color,
        } as CreateMillingOrderStatusDto);
      }
    });
  }
  ...
  delete(id: number, name: string): void {
    Swal.fire({
      ...
      text: `Bạn chuẩn bị xóa trạng thái lệnh xay "${name}". Thao tác này không thể hoàn tác!`,
      icon: 'warning',
      showCancelButton: true,
