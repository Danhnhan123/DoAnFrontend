...
import { RiceVarietyService } from '../../services/rice-variety.service';
...
export class RiceVarietyComponent {
  perm = inject(PermissionService);
  viewOnly = computed(() => this.isEdit() && !this.perm.canUpdate('RICE_VARIETIES'));
  private riceVarietyService = inject(RiceVarietyService);
  ...
  editItem = signal<RiceVarietyRow | null>(null);
  ...
  listQuery = injectQuery(() => ({
    ...
    queryFn: () => {
      const body = this.riceVarietyService.buildPagedBody({
        page: this.page(),
      ...
      return lastValueFrom(this.riceVarietyService.getPagedAdvanced(body));
    },
  }));

  detailQuery = injectQuery(() => ({
    queryKey: ['rice-variety-detail', this.editItem()?.id],
    enabled: !!this.editItem()?.id && this.showModal(),
    queryFn: () =>
      lastValueFrom(this.riceVarietyService.getById(this.editItem()!.id)),
  }));

  rows = computed<RiceVarietyRow[]>(() => {
  ...
  private syncDetail = effect(() => {
    ...
    const detail: RiceVarietyDetailDto =
    ...
    this.form.set({
      ...
      defaultYieldRate: detail.defaultYieldRate ?? null,
      note: detail.note || '',
  ...
  createMutation = injectMutation(() => ({
    mutationFn: (payload: CreateRiceVarietyDto) =>
      lastValueFrom(this.riceVarietyService.create(payload)),
    onSuccess: (res: any) => {
      if (res.isSucceeded) {
        this.closeModal();
        this.queryClient.invalidateQueries({ queryKey: ['rice-varieties'] });
        this.showAlert('Thêm giống lúa thành công!');
      } else {
  ...
  updateMutation = injectMutation(() => ({
    mutationFn: (payload: UpdateRiceVarietyDto) =>
      lastValueFrom(this.riceVarietyService.update(payload)),
    onSuccess: (res: any) => {
      if (res.isSucceeded) {
        this.closeModal();
        this.queryClient.invalidateQueries({ queryKey: ['rice-varieties'] });
        this.showAlert('Cập nhật giống lúa thành công!');
      } else {
  ...
  deleteMutation = injectMutation(() => ({
    mutationFn: (id: number) =>
      lastValueFrom(this.riceVarietyService.delete(id)),
    onSuccess: (res: any) => {
      if (res.isSucceeded) {
        this.queryClient.invalidateQueries({ queryKey: ['rice-varieties'] });
        this.showAlert('Đã xóa giống lúa!');
      } else {
  ...
  openEdit(row: RiceVarietyRow): void {
    this.editItem.set(row);
    this.form.set({
      name: row.name || '',
      code: row.code || '',
      season: row.season || '',
      defaultYieldRate: row.defaultYieldRate ?? null,
      note: row.note || '',
  ...
  save(): void {
    ...
    if (!f.name?.trim() || !f.code?.trim()) {
      this.showAlert('Vui lòng nhập Tên và Mã giống lúa', false);
      return;
    }

    const yield_ =
      f.defaultYieldRate === null ||
      f.defaultYieldRate === '' ||
      f.defaultYieldRate === undefined
        ? null
        : Number(f.defaultYieldRate);

    const actionText = this.isEdit() ? 'cập nhật' : 'thêm mới';
    Swal.fire({
      title: `Xác nhận ${actionText}`,
      text: `Bạn có muốn ${actionText} giống lúa này không?`,
      icon: 'question',
      ...
      const base: CreateRiceVarietyDto = {
        name: f.name.trim(),
        code: f.code.trim(),
        season: f.season?.trim() || null,
        defaultYieldRate: yield_,
        note: f.note?.trim() || null,
        isActive: !!f.isActive,
      };

      if (this.isEdit()) {
        const payload: UpdateRiceVarietyDto = {
          ...base,
          id: this.editItem()!.id,
        };
