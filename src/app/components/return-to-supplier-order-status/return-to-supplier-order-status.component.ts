...
import {
  ReturnToSupplierOrderStatusAdvancedRow,
  ReturnToSupplierOrderStatusDetailDto,
  CreateReturnToSupplierOrderStatusDto,
  UpdateReturnToSupplierOrderStatusDto,
} from '../../models';
import { ReturnToSupplierOrderStatusService } from '../../services/return-to-supplier-order-status.service';
...
@Component({
  selector: 'app-return-to-supplier-order-status',
  standalone: true,
  imports: [ReadonlyIfDirective, HasPermissionDirective, CommonModule, FormsModule, FilterSelectComponent],
  templateUrl: './return-to-supplier-order-status.component.html',
  styleUrl: './return-to-supplier-order-status.component.css',
})
export class ReturnToSupplierOrderStatusComponent {
  perm = inject(PermissionService);
  viewOnly = computed(() => this.isEdit() && !this.perm.canUpdate('RETURN_TO_SUPPLIER_ORDER_STATUS'));
  private svc = inject(ReturnToSupplierOrderStatusService);
  ...
  editItem = signal<ReturnToSupplierOrderStatusAdvancedRow | null>(null);
  ...
  detailQuery = injectQuery(() => ({
    queryKey: ['return-to-supplier-order-status-detail', this.editItem()?.id],
    enabled: !!this.editItem()?.id && this.showModal(),
    queryFn: () => lastValueFrom(this.svc.getById(this.editItem()!.id)),
  }));

  rows = computed<ReturnToSupplierOrderStatusAdvancedRow[]>(() => {
  ...
  get _detailSynced(): boolean {
    ...
    if (d && d !== this._prevDetailData) {
      ...
      const detail: ReturnToSupplierOrderStatusDetailDto = (d as any)?.resources ?? (d as any)?.data;
  ...
  createMutation = injectMutation(() => ({
    mutationFn: (payload: CreateReturnToSupplierOrderStatusDto) => lastValueFrom(this.svc.create(payload)),
    onSuccess: (res: any) => {
      if (res.isSucceeded) {
        this.closeModal();
        this.queryClient.invalidateQueries({ queryKey: ['return-to-supplier-order-status'] });
  ...
  updateMutation = injectMutation(() => ({
    mutationFn: (payload: UpdateReturnToSupplierOrderStatusDto) => lastValueFrom(this.svc.update(payload)),
    onSuccess: (res: any) => {
      if (res.isSucceeded) {
        this.closeModal();
        this.queryClient.invalidateQueries({ queryKey: ['return-to-supplier-order-status'] });
  ...
  openEdit(row: ReturnToSupplierOrderStatusAdvancedRow): void {
    this._prevDetailData = null;
  ...
  save(): void {
    ...
    if (!f.name) { this.showAlert('Vui lòng điền đầy đủ tên trạng thái đơn trả NCC (*)', false); return; }
    const actionText = this.isEdit() ? 'cập nhật' : 'thêm mới';
    Swal.fire({
      title: `Xác nhận ${actionText}`,
      text: `Bạn có chắc chắn muốn ${actionText} trạng thái đơn trả NCC này?`,
      icon: 'question',
      ...
      if (this.isEdit()) {
        this.updateMutation.mutate({
          ...
          name: f.name,
          color: f.color,
        } as UpdateReturnToSupplierOrderStatusDto);
      } else {
        this.createMutation.mutate({
          code: f.code,
          name: f.name,
          color: f.color,
        } as CreateReturnToSupplierOrderStatusDto);
      }
    });
  }
  ...
  delete(id: number, name: string): void {
    Swal.fire({
      ...
      text: `Bạn chuẩn bị xóa trạng thái đơn trả NCC "${name}". Thao tác này không thể hoàn tác!`,
      icon: 'warning',
      showCancelButton: true,
