...
export class InboundPutawayComponent implements OnInit {
  ...
  readonly lines = signal<InboundPutawayLine[]>([]);
  readonly locations = signal<LocationDetailDto[]>([]);
  ...
  readonly proposedOccupancy = computed(() => {
    ...
    const max =
      suggestion.currentOccupancy + suggestion.availableCapacity;
    if (max <= 0) return 0;
  ...
  async advanceOrder(): Promise<void> {
    ...
    try {
      ...
      if (status === 'draft') {
        ...
        await this.success('Đã gửi phiếu nhập kho để duyệt.');
      } else if (status === 'submitted') {
        await lastValueFrom(this.inboundService.approve(line.order.id));
        await this.success('Đã phê duyệt phiếu nhập kho.');
      } else if (status === 'approved') {
        await lastValueFrom(
          this.inboundService.startReceipt(line.order.id, line.item.id)
        );
        await this.success('Đã bắt đầu quá trình nhận hàng.');
      }
    ...
    try {
      ...
      if (selected) {
        ...
        this.activeStatus.set(
          selected.canFitWhole ? 'suggested' : 'split'
        );
        this.message.set(
          preferredLocationId &&
  ...
  async confirmPutaway(): Promise<void> {
    ...
    try {
      await lastValueFrom(
        this.inboundService.selectPutaway(line.order.id, line.item.id, {
          ...
          overrideReason: this.manualOverride()
            ...
            : null,
          weightKg: Number(this.placementWeightKg()),
        })
      );
      await lastValueFrom(
      ...
      await this.success(
        this.placementWeightKg() < this.remaining(line.item)
