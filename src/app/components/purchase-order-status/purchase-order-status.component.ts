...
import {
  PurchaseOrderStatusAdvancedRow,
...
@Component({
  ...
  styleUrl: './purchase-order-status.component.css',
})
export class PurchaseOrderStatusComponent {
  perm = inject(PermissionService);
  viewOnly = computed(() => this.isEdit() && !this.perm.canUpdate('PURCHASE_ORDER_STATUS'));
  private svc = inject(PurchaseOrderStatusService);
  ...
  editItem = signal<PurchaseOrderStatusAdvancedRow | null>(null);
  ...
  detailQuery = injectQuery(() => ({
    queryKey: ['purchase-order-status-detail', this.editItem()?.id],
    enabled: !!this.editItem()?.id && this.showModal(),
    queryFn: () => lastValueFrom(this.svc.getById(this.editItem()!.id)),
  }));

  rows = computed<PurchaseOrderStatusAdvancedRow[]>(() => {
  ...
  get _detailSynced(): boolean {
    ...
    if (d && d !== this._prevDetailData) {
      ...
      const detail: PurchaseOrderStatusDetailDto = (d as any)?.resources ?? (d as any)?.data;
  ...
  createMutation = injectMutation(() => ({
    mutationFn: (payload: CreatePurchaseOrderStatusDto) => lastValueFrom(this.svc.create(payload)),
    onSuccess: (res: any) => {
      if (res.isSucceeded) {
        this.closeModal();
        this.queryClient.invalidateQueries({ queryKey: ['purchase-order-status'] });
  ...
  updateMutation = injectMutation(() => ({
    mutationFn: (payload: UpdatePurchaseOrderStatusDto) => lastValueFrom(this.svc.update(payload)),
    onSuccess: (res: any) => {
      if (res.isSucceeded) {
        this.closeModal();
        this.queryClient.invalidateQueries({ queryKey: ['purchase-order-status'] });
  ...
  openEdit(row: PurchaseOrderStatusAdvancedRow): void {
    this._prevDetailData = null;
  ...
  save(): void {
    const f = this.form();
    if (!f.name) { this.showAlert('Vui lòng điền đầy đủ tên trạng thái đơn mua (*)', false); return; }
    const actionText = this.isEdit() ? 'cập nhật' : 'thêm mới';
    Swal.fire({
      title: `Xác nhận ${actionText}`,
      text: `Bạn có chắc chắn muốn ${actionText} trạng thái đơn mua này?`,
      icon: 'question',
      ...
      if (this.isEdit()) {
        this.updateMutation.mutate({
          ...
          name: f.name,
          color: f.color,
        } as UpdatePurchaseOrderStatusDto);
      } else {
        this.createMutation.mutate({
          name: f.name,
          color: f.color,
        } as CreatePurchaseOrderStatusDto);
      }
    });
  }
  ...
  delete(id: number, name: string): void {
    Swal.fire({
      ...
      text: `Bạn chuẩn bị xóa trạng thái đơn mua "${name}". Thao tác này không thể hoàn tác!`,
      icon: 'warning',
      showCancelButton: true,
