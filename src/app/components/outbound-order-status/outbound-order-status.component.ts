...
import {
  OutboundOrderStatusAdvancedRow,
  OutboundOrderStatusDetailDto,
  CreateOutboundOrderStatusDto,
  UpdateOutboundOrderStatusDto,
} from '../../models';
import { OutboundOrderStatusService } from '../../services/outbound-order-status.service';
...
@Component({
  selector: 'app-outbound-order-status',
  standalone: true,
  imports: [ReadonlyIfDirective, HasPermissionDirective, CommonModule, FormsModule, FilterSelectComponent],
  templateUrl: './outbound-order-status.component.html',
  styleUrl: './outbound-order-status.component.css',
})
export class OutboundOrderStatusComponent {
  perm = inject(PermissionService);
  viewOnly = computed(() => this.isEdit() && !this.perm.canUpdate('OUTBOUND_ORDER_STATUS'));
  private svc = inject(OutboundOrderStatusService);
  ...
  editItem = signal<OutboundOrderStatusAdvancedRow | null>(null);
  ...
  detailQuery = injectQuery(() => ({
    queryKey: ['outbound-order-status-detail', this.editItem()?.id],
    enabled: !!this.editItem()?.id && this.showModal(),
    queryFn: () => lastValueFrom(this.svc.getById(this.editItem()!.id)),
  }));

  rows = computed<OutboundOrderStatusAdvancedRow[]>(() => {
  ...
  get _detailSynced(): boolean {
    ...
    if (d && d !== this._prevDetailData) {
      ...
      const detail: OutboundOrderStatusDetailDto = (d as any)?.resources ?? (d as any)?.data;
  ...
  createMutation = injectMutation(() => ({
    mutationFn: (payload: CreateOutboundOrderStatusDto) => lastValueFrom(this.svc.create(payload)),
    onSuccess: (res: any) => {
      if (res.isSucceeded) {
        this.closeModal();
        this.queryClient.invalidateQueries({ queryKey: ['outbound-order-status'] });
  ...
  updateMutation = injectMutation(() => ({
    mutationFn: (payload: UpdateOutboundOrderStatusDto) => lastValueFrom(this.svc.update(payload)),
    onSuccess: (res: any) => {
      if (res.isSucceeded) {
        this.closeModal();
        this.queryClient.invalidateQueries({ queryKey: ['outbound-order-status'] });
  ...
  openEdit(row: OutboundOrderStatusAdvancedRow): void {
    this._prevDetailData = null;
  ...
  save(): void {
    ...
    if (!f.name) { this.showAlert('Vui lòng điền đầy đủ tên trạng thái đơn xuất (*)', false); return; }
    const actionText = this.isEdit() ? 'cập nhật' : 'thêm mới';
    Swal.fire({
      title: `Xác nhận ${actionText}`,
      text: `Bạn có chắc chắn muốn ${actionText} trạng thái đơn xuất này?`,
      icon: 'question',
      ...
      if (this.isEdit()) {
        this.updateMutation.mutate({
          ...
          name: f.name,
          color: f.color,
        } as UpdateOutboundOrderStatusDto);
      } else {
        this.createMutation.mutate({
          code: f.code,
          name: f.name,
          color: f.color,
        } as CreateOutboundOrderStatusDto);
      }
    });
  }
  ...
  delete(id: number, name: string): void {
    Swal.fire({
      ...
      text: `Bạn chuẩn bị xóa trạng thái đơn xuất "${name}". Thao tác này không thể hoàn tác!`,
      icon: 'warning',
      showCancelButton: true,
