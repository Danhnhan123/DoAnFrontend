...
import {
  PaddyPurchaseScheduleStatusAdvancedRow,
  PaddyPurchaseScheduleStatusDetailDto,
  CreatePaddyPurchaseScheduleStatusDto,
  UpdatePaddyPurchaseScheduleStatusDto,
} from '../../models';
import { PaddyPurchaseScheduleStatusService } from '../../services/paddy-purchase-schedule-status.service';
...
@Component({
  selector: 'app-paddy-purchase-schedule-status',
  standalone: true,
  imports: [ReadonlyIfDirective, HasPermissionDirective, CommonModule, FormsModule, FilterSelectComponent],
  templateUrl: './paddy-purchase-schedule-status.component.html',
  styleUrl: './paddy-purchase-schedule-status.component.css',
})
export class PaddyPurchaseScheduleStatusComponent {
  perm = inject(PermissionService);
  viewOnly = computed(() => this.isEdit() && !this.perm.canUpdate('PADDY_PURCHASE_SCHEDULE_STATUS'));
  private svc = inject(PaddyPurchaseScheduleStatusService);
  ...
  editItem = signal<PaddyPurchaseScheduleStatusAdvancedRow | null>(null);
  ...
  detailQuery = injectQuery(() => ({
    queryKey: ['paddy-purchase-schedule-status-detail', this.editItem()?.id],
    enabled: !!this.editItem()?.id && this.showModal(),
    queryFn: () => lastValueFrom(this.svc.getById(this.editItem()!.id)),
  }));

  rows = computed<PaddyPurchaseScheduleStatusAdvancedRow[]>(() => {
  ...
  get _detailSynced(): boolean {
    ...
    if (d && d !== this._prevDetailData) {
      ...
      const detail: PaddyPurchaseScheduleStatusDetailDto = (d as any)?.resources ?? (d as any)?.data;
  ...
  createMutation = injectMutation(() => ({
    mutationFn: (payload: CreatePaddyPurchaseScheduleStatusDto) => lastValueFrom(this.svc.create(payload)),
    onSuccess: (res: any) => {
      if (res.isSucceeded) {
        this.closeModal();
        this.queryClient.invalidateQueries({ queryKey: ['paddy-purchase-schedule-status'] });
  ...
  updateMutation = injectMutation(() => ({
    mutationFn: (payload: UpdatePaddyPurchaseScheduleStatusDto) => lastValueFrom(this.svc.update(payload)),
    onSuccess: (res: any) => {
      if (res.isSucceeded) {
        this.closeModal();
        this.queryClient.invalidateQueries({ queryKey: ['paddy-purchase-schedule-status'] });
  ...
  openEdit(row: PaddyPurchaseScheduleStatusAdvancedRow): void {
    this._prevDetailData = null;
  ...
  save(): void {
    const f = this.form();
    if (!f.name) { this.showAlert('Vui lòng điền đầy đủ tên trạng thái lịch thu mua (*)', false); return; }
    const actionText = this.isEdit() ? 'cập nhật' : 'thêm mới';
    Swal.fire({
      title: `Xác nhận ${actionText}`,
      text: `Bạn có chắc chắn muốn ${actionText} trạng thái lịch thu mua này?`,
      icon: 'question',
      ...
      if (this.isEdit()) {
        this.updateMutation.mutate({
          ...
          name: f.name,
          color: f.color,
        } as UpdatePaddyPurchaseScheduleStatusDto);
      } else {
        this.createMutation.mutate({
          code: f.code,
          name: f.name,
          color: f.color,
        } as CreatePaddyPurchaseScheduleStatusDto);
      }
    });
  }
  ...
  delete(id: number, name: string): void {
    Swal.fire({
      ...
      text: `Bạn chuẩn bị xóa trạng thái lịch thu mua "${name}". Thao tác này không thể hoàn tác!`,
      icon: 'warning',
      showCancelButton: true,
